...
export interface IWorkoutSessionRepository {
  ...
  findAll(filter: Record<string, unknown>): Promise<IWorkoutSession[]>;
  findSessions(query: Record<string, unknown>, page: number, limit: number): Promise<{ sessions: IWorkoutSession[], total: number }>;
