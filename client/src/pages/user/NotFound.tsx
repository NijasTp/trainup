...
import type { TargetCursorProps } from "@/interfaces/user/INotFound";
...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  const isMobile = useMemo(() => {
    ...
    const userAgent = navigator.userAgent || navigator.vendor || (window as SafeAny).opera;
  ...
  useEffect(() => {
    ...
    // Check if corners can be selected, if not retry in a moment (React rendering)
    if (!cursor.querySelector('.target-cursor-corner')) return;
      ...
      const leaveHandler = () => {
        ...
        resumeTimeout = setTimeout(() => {
          if (!activeTarget && cursorRef.current && spinTl.current) {
            ...
            spinTl.current.kill(); // Kill previous
            spinTl.current = gsap
              .timeline({ repeat: -1 })
...
const NotFound = () => {
  ...
  return (
    <div className="flex h-screen w-full items-center justify-center bg-black overflow-hidden relative">
      ...
      <div className="flex flex-col items-center justify-center z-10 text-center space-y-8">
        <h1 className="text-white text-[120px] font-black leading-none tracking-tighter cursor-target select-none">
          404
        </h1>
        <h2 className="text-white text-3xl font-bold tracking-widest uppercase cursor-target">
          THIS PAGE IS NOT AVAILABLE
        </h2>
        <Button
          onClick={() => navigate('/')}
          className="cursor-target bg-white text-black hover:bg-gray-200 px-8 py-6 text-lg font-bold tracking-wider rounded-none uppercase transition-all duration-300"
        >
          Go Back Home
        </Button>
      </div>
      ...
      {/* Background Elements */}
      <div className="absolute inset-0 pointer-events-none opacity-20 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-gray-500 via-transparent to-transparent" />
