...
import { Search, ArrowLeft, Apple, Star, Clock, Trophy } from "lucide-react"
...
export default function TrainerAssignDiets() {
  ...
  const fetchTemplates = useCallback(async () => {
    setIsLoading(true)
    try {
      const response = await API.get("/template/diet", {
        params: { limit: 100 }
      })
  ...
  const handleAssign = async (templateId: string) => {
    ...
    try {
      await API.post("/template/diet/assign", { userId, templateId })
      ...
      <main className="relative container mx-auto px-6 py-12 space-y-12 z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-4">
            <Button 
                variant="ghost" 
                onClick={() => navigate(-1)}
                className="group flex items-center gap-2 text-white/40 hover:text-green-400 p-0 hover:bg-transparent"
            >
                <ArrowLeft className="h-4 w-4 transition-transform group-hover:-translate-x-1" />
                <span className="text-[10px] font-black uppercase italic tracking-widest">Back to Network</span>
            </Button>
            <div className="space-y-2">
                <Badge className="bg-green-500/10 text-green-400 border-green-500/20 px-3 py-1 font-black italic uppercase tracking-widest text-[10px]">
                    Nutritional Deployment
                </Badge>
                <h1 className="text-5xl md:text-6xl font-black italic uppercase tracking-tighter leading-none">
                    Assign <span className="text-green-500">Diet</span>
                </h1>
                <p className="text-white/40 font-medium text-lg max-w-xl">
                    Deploying metabolic optimization protocols for <span className="text-white font-bold">{client?.name || "Target User"}</span>.
                </p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
            <Button 
                onClick={() => navigate("/trainer/templates/new/diet")}
            ...
            <div className="relative w-full md:w-80">
                ...
                className="pl-11 bg-white/[0.03] border-white/10 focus:border-green-500/50 focus:ring-1 focus:ring-green-500/20 h-14 rounded-2xl text-sm font-bold placeholder:text-white/10 italic"
                />
            </div>
          </div>
        </div>
        ...
        {isLoading ? (
            <div className="flex flex-col items-center justify-center py-24 space-y-4">
                <div className="w-12 h-12 border-2 border-white/5 border-t-green-500 rounded-full animate-spin" />
                <p className="text-[10px] font-black uppercase italic tracking-widest text-white/20">Accessing Nutrient Database...</p>
            </div>
        ) : (
                ...
                {filteredTemplates.map((template) => (
                    ...
                    >
                        <div className="aspect-video relative overflow-hidden">
                            ...
                            <Badge className="absolute top-4 right-4 bg-black/60 backdrop-blur-md border-white/10 text-green-400 font-black italic uppercase tracking-widest text-[9px] px-3 py-1">
                                {template.goal}
                            </Badge>
                        </div>
                        ...
                        <CardContent className="p-8 space-y-6 -mt-8 relative z-10">
                            <div className="space-y-2">
                                <Badge className="bg-green-500/10 text-green-400 border-green-500/20 text-[9px] font-black italic uppercase">
                                    {template.duration} Day Protocol
                                </Badge>
                                <h3 className="text-2xl font-black italic uppercase tracking-tighter group-hover:text-green-400 transition-colors uppercase">
                                    {template.title}
                                </h3>
                            ...
                            <div className="flex items-center gap-6 pt-4 border-t border-white/5">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-black text-white/60 italic uppercase">{template.duration} Days</span>
                                </div>
                            </div>

                            <Button
                                className={cn(
                                    "w-full h-14 bg-white text-black hover:bg-green-500 hover:text-white font-black italic uppercase tracking-widest text-xs rounded-2xl transition-all duration-300",
        ...
        {!isLoading && filteredTemplates.length === 0 && (
            <div className="text-center py-24 space-y-6 opacity-40">
                <Apple className="h-16 w-16 mx-auto text-white/10" />
                <div className="space-y-2">
                    <h3 className="text-2xl font-black italic uppercase tracking-tighter">No Nutrients Found</h3>
